    ...
    describe "Invalid Arguments" do
      [{ category: 'sales' }, { location: 'london' }].each do |arg|
        it "should not contain only #{arg.keys.first} as input" do
          expect{JobFinder::Crawler.new(JobFinder::Platform::INDEED, arg).crawl}.to raise_error(JobFinder::UrlException)
        end
      end
      it 'should contain valid page number' do
        args = { category: 'sales', location: 'london', page: '1a' }
        expect{JobFinder::Crawler.new(JobFinder::Platform::INDEED, args).crawl}.to raise_error(JobFinder::PageNumberException)
      end
